...
<!-- 회원가입 로그인 모달 CSS, JS -->
<link href="<c:url value="/css/member/login-register.css"/>"
	rel="stylesheet" />
<link href=" <c:url value="/css/member/bootstrap.css"/>"
	rel="stylesheet" />
<link rel="stylesheet"
	href="http://netdna.bootstrapcdn.com/font-awesome/4.0.3/css/font-awesome.css">

<script src="<c:url value="/js/member/login-register.js"/>"
	type="text/javascript"></script>
<script src="<c:url value="/js/member/bootstrap.js"/>"
	type="text/javascript"></script>
		...
		<div class="memContent">
				...
				<div class="row">
					...
					<div class="col-sm-4">
						...
						<a class="btn big-register" data-toggle="modal"
							href="javascript:void(0)" onclick="openRegisterModal();">회원
							가입</a>
					</div>
					<div class="col-sm-4"></div>
				</div>
						...
						<div class="modal-content">
								...
								<div class="box">
									<div class="content-modal">
										...
										<div class="social">
											...
											</a> <a id="facebook_login" class="circle facebook" href="#">
										...
										<div class="form loginBox">
											<form method="post" action="" accept-charset="UTF-8">
												<input id="memId" class="form-control" type="text"
													placeholder="Email (ID로 사용됩니다.)" name="memId"> <input
													id="memPw" class="form-control" type="password"
													placeholder="Password" name="memPw"> <input
													class="btn btn-default btn-login" type="button"
													value="Login" name="loginButton" id="loginButton">
											</form>
										</div>
									...
									<div class="registerBox" style="display: none;">
										<div class="form">
											<form method="post" html="{:multipart=>true}"
												...
												<input id="memIdReg" class="form-control" type="text"
													placeholder="Email" name="memIdReg"> <input
													id="memPwReg" class="form-control" type="password"
													placeholder="비밀번호(4자 이상 12자 이하)" name="memPwReg"> <input
													id="password_confirmation" class="form-control"
													type="password" placeholder="비밀번호 확인"
													name="password_confirmation"> <input id="nickName"
													class="form-control" type="text" placeholder="이름(닉네임)"
													name="nickName"> <select id="genderSelect"
													class="form-control-select" name="memGender">
													<option value="">성별</option>
													<option value="M">남성</option>
													<option value="F">여성</option>
													<option value="N">선택하지 않음</option>
												</select> <input class="btn btn-default btn-register" type="button"
													value="가입 하기" name="commit" id="commit">
											</form>
										</div>
									</div>
								</div>
							...
							<div class="modal-footer">
								<div class="forgot login-footer">
									<span>계정이 없으신가요? <a
										href="javascript: showRegisterForm();">회원가입 하러가기</a>
									</span>
								</div>

								<div class="forgot register-footer" style="display: none">
									<span>이미 계정을 가지고 계신가요?</span> <a
										href="javascript: showLoginForm();">로그인 하러가기</a>
								</div>
			...
			<div class="mypage-menu">
				...
				<div class="mypage-header">
					...
					<div class="mem-mail-state" id="memMailState">
					${msg}
					</div>
				...
				<div class="mypage-body-1">
					...
					<div class="mem-change" data-toggle="modal"
						data-target="#mypageModal" data-whatever="내 정보 변경">내 정보 변경</div>

					<hr class="mypage-hr">

					<div class="mem-locchange" data-toggle="modal"
						data-target="#mypageModal" data-whatever="내 위치 변경">내 위치 변경
						</div>

					<hr class="mypage-hr">

					<div class="mem-photochange" data-toggle="modal"
						data-target="#mypageModal" data-whatever="프로필 사진 등록 변경">프로필
						사진 등록 변경</div>
				...
				<div class="mypage-body-2">
					...
					<div class="mem-like" data-toggle="modal"
						data-target="#mypageModal" data-whatever="좋아요 한 게시물">좋아요 한
						게시물</div>

					<hr class="mypage-hr">

					<div class="mem-bookmark" data-toggle="modal"
						data-target="#mypageModal" data-whatever="북마크 한 게시물">북마크 한
						게시물</div>
				...
				<div class="mypage-body-3">
					...
					<div class="mem-notice" data-toggle="modal"
						data-target="#mypageModal" data-whatever="공지사항">공지사항</div>

					<hr class="mypage-hr">

					<div class="mem-delete" data-toggle="modal"
						data-target="#mypageModal" data-whatever="회원 탈퇴">회원 탈퇴</div>
...
<!-- 	<script type="text/javascript">
	...
	<script>
		$('#mypageModal').on('show.bs.modal',function(event) {
							var mypageDiv = $(event.relatedTarget) // Button that triggered the modal
							var recipient = mypageDiv.data('whatever') // Extract info from data-* attributes
						...
						$('.mem-change').click(function(){
							...
							$('.mem-delete').click(function(){
								...
								memberDeleteHtml += '<img src="<c:url value="/fileupload/member/"/>">';
								memberDeleteHtml += '<h2>${loginInfo.memName} 님</h2>';
							...
							$('.mem-photochange').click(function(){
								...
								memberPhotoHtml +='<h1>프로필 사진 등록 & 변경 </h1>';
								...
								memberPhotoHtml +='<form method="post" action="photoUpload" enctype="multipart/form-data">';
								memberPhotoHtml +='<input type="file" id="memPhoto" name="memPhoto" class="mem-upload-photo" value="사진 업로드">';
							...
							$.ajax({
										...
										success : function(loginDone) {
											...
											if (memName == 'admin') {
												...
												loginHtml += '<div class="mem-info-name">'
														+ memName + '</div>';
												loginHtml += '<div class="mem-info-loc">'
														+ memIdx + ' 종로구</div>';
												loginHtml += '</div>';
